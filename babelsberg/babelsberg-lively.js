...
'plugin interface', {
    ignoreProp: function(obj, propName, value) {
        var self = this;
        if (propName == "__constrainedVariables__") {
    ...
    serializationDone: function(registry) {
        ...
        this.constraints.each(function(constraint) {
            var cvar =  constraint.constraintvariables.first() // oonly store in the first object
            var persistentCopy = serializer.getRegisteredObjectFromId(serializer.getIdFromObject(cvar.obj));
            ...
            if (!constraint._options) {
                console.log("Could not serialize constraint without options: " + constraint)
    ...
    deserializationDone: function() {
        ...
        this.constraints.each(function(constraint) {
            ...
            Properties.own(constraint.ctx).each(function(name){
                ...
                if (id) {
                    var msg = "Could not recreate constraint, because object " + name 
                        + " (" + id + ") was not found"
                    throw new Error(msg)
                }
                bindings[name] = obj
            })
            bbb.always({ctx: bindings, solver: solver}, eval("(" + constraint.predicate + ")"))
            } catch(e) {
...
Object.extend(SerializeConstraintsPlugin, {
     cleanObject: function(obj, visited, exclude) {
        ...
        try {
            if (obj.__constraints) { console.log("visit " + obj) }
